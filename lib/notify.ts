...
function buildBody(lead: Lead) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#16243D;max-width:560px;margin:0 auto">
      <div style="background:#16243D;padding:20px 24px;border-radius:10px 10px 0 0">
        <div style="color:#6FBF3F;font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase">New Website Lead</div>
        <div style="color:#fff;font-size:20px;font-weight:800;margin-top:4px">Free Inspection Request</div>
      </div>
  ...
  const message: EmailMessage = {
    ...
    recipients: {
      ...
      ...(bccRecipients.length > 0 && {
        bcc: bccRecipients.map((address) => ({ address })),
      }),
    },
